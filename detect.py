...
def in_path(points, speed, angle):
    ...
    x_min = r_center - track_width / \
        2 if angle < 0 else np.sqrt(
            np.power(r_cf - track_width/2, 2) - np.power(y_max, 2))
    ...
    if graph:
        ...
        x1 = np.linspace(r_center - track_width / 2, np.sqrt(np.power(r_cf -
                         track_width/2, 2) - np.power(y_max, 2)), 100)  # left boundary
        x2 = np.linspace(r_center + track_width / 2, np.sqrt(np.power(r_cf +
                         track_width/2, 2) - np.power(y_max, 2)), 100)  # right boundary
        y1 = np.sqrt(np.power(r_cf - track_width / 2, 2) - np.power(x1, 2))
        y2 = np.sqrt(np.power(r_cf + track_width / 2, 2) - np.power(x2, 2))
        ax.plot(x1, y1)
        ax.plot(x2, y2)
        plt.show()
        plt.savefig('plt.png')
    ...
    return np.any(points[
        (points[:, 1] >= np.sqrt(
            np.power(points[:, 0], 2) + np.power(r_cf - track_width/2, 2)))
        &
        (points[:, 1] <= np.sqrt(
            np.power(points[:, 0], 2) + np.power(r_cf + track_width/2, 2)))
        &
...
print(in_path(np.asarray([[0, 0], [1, 1]], dtype=np.float64), 4, 45))
